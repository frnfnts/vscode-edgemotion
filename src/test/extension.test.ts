...
suite("EdgeMotion", () => {
  suite("EdgeMotion Cursor Movement", () => {
    test("MoveToNextEdge moves cursor to the next island", async () => {
    ...
    test("MoveToNextEdge moves cursor to the next shore", async () => {
      const document = await vscode.workspace.openTextDocument({
        content: content,
      });
      const editor = await vscode.window.showTextDocument(document);
    ...
    test("MoveToPreviousEdge moves cursor to the previous island", async () => {
      const document = await vscode.workspace.openTextDocument({
        content: content,
      });
      const editor = await vscode.window.showTextDocument(document);
    ...
    test("MoveToNextEdge moves cursor to the previous shore", async () => {
      const document = await vscode.workspace.openTextDocument({
        content: content,
      });
      const editor = await vscode.window.showTextDocument(document);
    ...
    test("MoveToPreviousEdge moves cursor to the next island", async () => {
      const document = await vscode.workspace.openTextDocument({
        content: content,
      });
      const editor = await vscode.window.showTextDocument(document);
      ...
      const testCases = [
        { startLine: 5, endLine: 1, startChar: 0, endChar: 0 },
        { startLine: 4, endLine: 2, startChar: 2, endChar: 2 },
        { startLine: 3, endLine: 3, startChar: 4, endChar: 4 },
      ];
